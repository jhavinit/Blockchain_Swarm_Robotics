...
* Logic: Stores the list of nodes in the blockchain. Sends a " b'client' " and then " b'add_nodes' " request to server-2.py so as to send the list of IP's and respective port numbers.  
...
* Input: transaction_pool: It contains the transaction pool recieved by main-2.py for adding it to blockchain
...
* Logic: During a transaction is being added into blockchain first a lock is created on the get_chain()(In server-2.py), because if transaction is added into the blockhain and some other
